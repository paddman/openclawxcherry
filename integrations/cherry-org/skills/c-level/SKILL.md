...
```bash
$HOME/.local/bin/cherry-org route "<คำถามหรือภารกิจของผู้ใช้>"
```

เรียกประชุมองค์กรและรับ Executive Memo:

```bash
$HOME/.local/bin/cherry-org meeting "<คำถามหรือภารกิจของผู้ใช้>" --requester openclaw
```

บังคับบทบาทเฉพาะเมื่อผู้ใช้ระบุชัด:

```bash
$HOME/.local/bin/cherry-org meeting "<คำถาม>" --agents shareholder_rep,ceo,cfo,cto
```
...
7. ถ้า service ล้มเหลว ให้รายงาน error จริง ห้ามจำลองผลประชุมขึ้นเอง
